...
public class AddTeamSpawnSubCommand extends SubCommand {
	...
	public AddTeamSpawnSubCommand(UltimateSheepWarsPlugin plugin) {
	...
	protected void onExePlayer(Player player, String... args) {
		if (!args[1].equalsIgnoreCase("red") && !args[1].equalsIgnoreCase("blue") && !args[1].equalsIgnoreCase("spec")) {
			...
			Hologram.runHologramTask(team.getColor() + "New team spawn added: N°" + team.getSpawns().size(), player.getLocation(), 5, this.getUltimateSheepWarsInstance());
		}
	}
