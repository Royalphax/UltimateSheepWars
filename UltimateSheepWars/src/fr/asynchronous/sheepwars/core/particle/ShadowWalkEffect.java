		...
		if (moving && under.getType() != Material.AIR) {
			if (left) {
				...
				if (under.getType() != Material.AIR) {
					SheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.SMOKE_LARGE, locationLeft, 0f, 0f, 0f, 2, 0f);
					SheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.FOOTSTEP, locationLeft, 0f, 0f, 0f, 1, 0f);
				}
			} else if (!left) {
				left = true;
				...
				if (under.getType() != Material.AIR) {
					SheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.SMOKE_LARGE, locationRight, 0f, 0f, 0f, 2, 0f);
					SheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.FOOTSTEP, locationRight, 0f, 0f, 0f, 1, 0f);
				}
			}
		}
	}
