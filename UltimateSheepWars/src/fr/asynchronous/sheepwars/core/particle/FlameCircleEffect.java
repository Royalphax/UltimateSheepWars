...
public class FlameCircleEffect implements ParticleEffectType {
	...
	public void update(Player player, Boolean moving) {
		if (!moving)
        {
            ...
            MathUtils.rotateVector(v, this.xRotation, this.yRotation, 
              this.zRotation);
            if (this.enableRotation) {
            	MathUtils.rotateVector(v, this.angularVelocityX * 
                this.step, this.angularVelocityY * this.step, 
                this.angularVelocityZ * this.step);
            }
            UltimateSheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.FLAME, location.add(v), 0.0F, 0.0F, 0.0F, 1, speed);
            this.step += 1.0F;
        }
        else
        {
        	UltimateSheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.FLAME, player.getLocation().clone().add(0,1,0), 0.2F, 0.2F, 0.2F, 1, speed);
        }
	}
}
