...
public class FlameSpiralEffect implements ParticleEffectType {
	...
	public void update(Player player, Boolean moving) {
		...
		if (!moving)
        {
          ...
          loc.add(v);
          UltimateSheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.FLAME, loc, 0.0f, 0.0f, 0.0f, 1, 0.0f);
          this.step += 1.0F;
        ...
        {
        	UltimateSheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.FLAME, loc, 0.2f, 0.2f, 0.2f, 1, 0.1f);
        }
	}
}
