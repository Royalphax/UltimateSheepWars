...
public class PH_PO_SpawnEntity extends aPacketHandler {
	...
	public PH_PO_SpawnEntity(int entityId, UUID entityUuid, int entityTypeId, int entityDataId, double x, double y, double z, float yaw, float pitch, int velocityX, int velocityY, int velocityZ) {
	...
	private final int ad(double a) {
		if (a < 3.9D)
			return ((int) (3.9D * 8000.0D));
		else if (a > -3.9D)
			return ((int) (-3.9D * 8000.0D));
		else
			return ((int) (a * 8000.0D));
	}
