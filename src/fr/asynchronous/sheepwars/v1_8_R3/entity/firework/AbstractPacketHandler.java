 ...
 public abstract class AbstractPacketHandler {
     ...
     static{
         try{
             ...
             AbstractPacketHandler.methodSendPacket=classPlayerConnection.getDeclaredMethod("sendPacket",
                     new Class[]{Class.forName("net.minecraft.server."+AbstractPacketHandler.mcVersion+".Packet")});
         }catch(Exception e){
             e.printStackTrace();
         }
     }
     ...
     */
     public final void send(Player...players){
         ...
         for(Player player:players)if(player!=null)AbstractPacketHandler.sendPacket(player, packet);
     }
     ...
     public static boolean sendPacket(Player player, Object packet){
         try{
             AbstractPacketHandler.methodSendPacket.invoke(AbstractPacketHandler.fieldPlayerConnection.get(
                     AbstractPacketHandler.methodGetHandle.invoke(player)), packet);
             return true;
         }catch(Exception e){
             e.printStackTrace();
             return false;
