...
public class EarthQuakeSheep extends SheepWarsSheep
{
    ...
    public EarthQuakeSheep() {
		super(Message.Messages.EARTHQUAKE_SHEEP_NAME, DyeColor.BROWN, 10, false, true, 0.8f);
	}
                ...
                for (int y = -RADIUS; y < RADIUS; ++y) {
                    for (int z = -RADIUS; z < RADIUS; ++z) {
                        ...
                        if (block.getType() != Material.AIR && top.getType() == Material.AIR && MathUtils.randomBoolean()) {
                            ...
                            SheepWarsPlugin.getVersionManager().getParticleFactory().playParticles(Particles.BLOCK_CRACK, topLocation, 0f, 0f, 0f, 1, 0f, (block.getType().getId()+(block.getData()<<12)));
                        }
                    }
                }
            }
