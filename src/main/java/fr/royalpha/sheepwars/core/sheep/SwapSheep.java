...
public class SwapSheep extends SheepWarsSheep {
	public SwapSheep() {
		super(Message.Messages.SWAP_SHEEP_NAME, DyeColor.MAGENTA, 5, false, true, 0.25f);
	}
	...
	public boolean onTicking(Player player, long ticks, Sheep bukkitSheep, Plugin plugin) {
		if (!bukkitSheep.hasMetadata("onGround")) {
			...
			if (nearest == null) {
				Message.sendMessage(player, Message.Messages.SWAP_SHEEP_ACTION_NOPLAYER);
				return true;
			}
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					if (this.ticks <= 0) {
						...
						Message.sendMessage(player, Message.Messages.SWAP_SHEEP_ACTION_TELEPORTATION);
						Message.sendMessage(nearestFinal, Message.Messages.SWAP_SHEEP_ACTION_TELEPORTATION);
						return;
