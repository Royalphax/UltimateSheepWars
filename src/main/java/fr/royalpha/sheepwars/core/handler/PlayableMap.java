...
import fr.royalpha.sheepwars.core.manager.WorldManager;
...
public class PlayableMap {
	...
	private double sheepVelocity = 2.0;
	...
	public void loadData() {
		if (!config.contains("team") && !config.contains("booster") && folder.getName().equals(WorldManager.LOBBY_MAP_NAME)) {
			for (VirtualLocation loc : ConfigManager.getLocations(ConfigManager.Field.BLUE_SPAWNS)) {
				teamSpawns.put(loc, SheepWarsTeam.BLUE);
			}
			for (VirtualLocation loc : ConfigManager.getLocations(ConfigManager.Field.RED_SPAWNS)) {
				teamSpawns.put(loc, SheepWarsTeam.RED);
			}
			for (VirtualLocation loc : ConfigManager.getLocations(ConfigManager.Field.SPEC_SPAWNS)) {
				teamSpawns.put(loc, SheepWarsTeam.SPEC);
			}
			for (VirtualLocation loc : ConfigManager.getLocations(ConfigManager.Field.BOOSTERS)) {
				boosterSpawns.add(loc);
			}
			sheepVelocity = ConfigManager.getDouble(ConfigManager.Field.SHEEP_VELOCITY);
		} else {
			...
			this.sheepVelocity = config.getDouble("sheep-velocity", 2.0);
		}
	}
	...
	public void uploadData() {
		...
		config.set("sheep-velocity", this.sheepVelocity);
		try {
			config.save(file);
		} catch (IOException e) {
			ExceptionManager.register(e, true);
	...
	public double getSheepVelocity() {
	...
	public void setSheepVelocity(double sheepVelocity) {
		this.sheepVelocity = sheepVelocity;
	}
