package fr.royalpha.sheepwars.v1_8_R3.entity.firework;
...
public class FireworkSpawner {
	...
	public static void spawn(Location location, FireworkEffect effect, ArrayList<Player> players) {
		...
		// Spawn
		new PacketHandlerSpawnEntity(entityId, 76/* firework entityId */, location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch(), 0, 0, 0, 0).send(players);

		// Set FireworkMeta
		ItemStack fireworkItem = new ItemStack(LegacyMaterial.FIREWORK.getMaterial());
		FireworkMeta meta = (FireworkMeta) fireworkItem.getItemMeta();
		meta.addEffect(effect);
		fireworkItem.setItemMeta(meta);

		scWatchableObject[] metadata = new scWatchableObject[] { new scWatchableObject(5, 8, FireworkSpawner.toNMSItemStack(fireworkItem)), new scWatchableObject(0, 4, (byte) 0), new scWatchableObject(0, 3, (byte) 0), new scWatchableObject(4, 2, ""), new scWatchableObject(1, 1, (short) 300), new scWatchableObject(0, 0, (byte) 0) };
	...
	static {
		try {
			FireworkSpawner.asNMSCopy = Class.forName("org.bukkit.craftbukkit." + Bukkit.getServer().getClass().getName().split("\\.")[3] + ".inventory.CraftItemStack").getDeclaredMethod("asNMSCopy", ItemStack.class);
		} catch (Exception a) {
			a.printStackTrace();
		}
	}
