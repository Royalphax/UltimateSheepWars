...
public abstract class SheepWarsSheep {
		...
		if (playerData.getTeam() != SheepWarsTeam.SPEC) {
			if (sheep.onGive(player)) {
				...
				if (!event.isCancelled()) {
					...
					if (amount > 1) {
						player.sendMessage(Message.getMessage(player, Message.Messages.SEVERAL_SHEEP_RECEIVED).replaceAll("%SHEEP_NAME%", sheep.getName(player)).replaceAll("%AMOUNT%", String.valueOf(amount)));
					} else {
						player.sendMessage(Message.getMessage(player, Message.Messages.SHEEP_RECEIVED).replaceAll("%SHEEP_NAME%", sheep.getName(player)));
					}
				}
			} else {
	...
	/**
	 * Use {@link #SheepWarsSheep(String, DyeColor, int, boolean, double, boolean, float, SheepAbility...) this constructor} instead.
	 */
	public SheepWarsSheep(final Message.Messages name, final DyeColor color, final int duration, final boolean friendly, final boolean drop, final float random, final SheepAbility... sheepAbilities) {
	 ...
	 * Use {@link #SheepWarsSheep(String, DyeColor, int, boolean, double, boolean, float, SheepAbility...) this constructor} instead.
	 */
	public SheepWarsSheep(final Message.Messages name, final DyeColor color, final int duration, final boolean friendly, final boolean drop, final SheepAbility... sheepAbilities) {
	...
	public boolean throwSheep(Player launcher, Plugin plugin) {
		...
		if (event.isCancelled()) {
			...
			if (!this.friendly) {
				...
				final PlayableMap map = PlayableMap.getPlayableMap(launcher.getWorld());
				if (map != null)
					entity.setVelocity(playerLocation.getDirection().multiply(map.getSheepVelocity()));
			}
