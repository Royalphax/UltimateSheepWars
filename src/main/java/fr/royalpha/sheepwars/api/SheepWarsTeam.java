...
public enum SheepWarsTeam {
	BLUE("blue", Message.getMessage(Message.Messages.BLUE_NAME), ConfigManager.getMaterial(ConfigManager.Field.TEAM_BLUE_MATERIAL), DyeColor.BLUE, ChatColor.BLUE, 85, 85, 255),
	RED("red", Message.getMessage(Message.Messages.RED_NAME), ConfigManager.getMaterial(ConfigManager.Field.TEAM_RED_MATERIAL), DyeColor.RED, ChatColor.RED, 255, 50, 50),
	SPEC("spec", Message.getMessage(Message.Messages.SPEC_NAME), Material.STONE, DyeColor.GRAY, ChatColor.WHITE, 0, 0, 0),
	...
	public void updateScoreboardTeamCount() {
		for (Language langs : Language.getLanguages())
			langs.getScoreboardWrapper().setLine((this == SheepWarsTeam.RED ? redSlot : blueSlot), langs.getMessage((this == SheepWarsTeam.RED ? Message.getMessage(Message.Messages.SCOREBOARD_TEAM_RED) : Message.getMessage(Message.Messages.SCOREBOARD_TEAM_BLUE))).replaceAll("%SIZE%", this.players.size() + ""), true);
	}
	...
	public Location getNextSpawn() {
		...
		if (spawns.isEmpty())
			return ConfigManager.getLocation(ConfigManager.Field.LOBBY).toBukkitLocation();
	...
	public ItemStack getIcon(Player p) {
		...
		item.setName((this == BLUE ? "" + Message.getMessage(p, ChatColor.BLUE + "", Message.Messages.JOIN_BLUE_ITEM, "") : "" + Message.getMessage(p, ChatColor.RED + "", Message.Messages.JOIN_RED_ITEM, "")));
