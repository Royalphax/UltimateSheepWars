...
public class FireworkSpawner {
	...
	public static void spawn(Location location, FireworkEffect effect, ArrayList<Player> players) {
		if (location == null || effect == null || players == null || players.size() == 0)
			return;
		...
		new PH_PO_SpawnEntity(entityId, entityUuid, 76/* ID of firework entity type */, 0, location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch(), 0, 0, 0).send(players);
		...
		try {
			// Send firework meta
			ItemStack item = new ItemStack(LegacyMaterial.FIREWORK.getMaterial());
			...
			jDataWatcherItem<Boolean> i51 = new jDataWatcherItem<Boolean>(fr.royalpha.sheepwars.v1_12_R1.entity.firework.jDataWatcherObject.entity_aD, false);
